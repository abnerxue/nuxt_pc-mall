...
<div>
        ...
        <!-- <?php foreach ($this->address as $k=>$address): ?> -->
                    <li >
                        <!-- <div class="user-name" addressid="<?php echo $address->id;?>"> -->
                            ...
                            <p><i style="background:url(../../static/img/address.jpg)"></i>寄送至</p>
                            <div class="address-info">
                                <span class="j-address">
                                    {{address[this.index].province+address[this.index].city+address[this.index].county+address[this.index].address}}
                        ...
                        <a class="changeAdd" href="javascript:;" @click='edit_address()'>修改</a>
  ...
  <el-dropdown-menu slot="dropdown" >
    <el-dropdown-item v-for='(item,i) in address' :key='i' v-model='choose' @click.native="chooseadd(i)">{{item.province+item.city+item.county+item.address+item.name+item.phone}}</el-dropdown-item>
        ...
        <!-- <?php else: ?> -->
			<div style="text-align:center;font-size:28px;margin-top:-29px;color:rgb(116, 24, 27)" v-if="address==''">暂无收获地址，请添加收货地址</div>
        <!-- <?php endif; ?> -->
		</div>
          <el-dialog
          left
         ...
         <tr class="order-item" v-if='this.$route.query.tag!=5'>
            <td>
                <div>
                    ...
                    <p>
                        <!-- <?php echo $goods->title;?> -->{{data.title}}
                        </p>
                </div>
            </td>
            <td><span>¥ </span>
            ...
            <td><span>¥{{data.price*this.$route.query.num}}</span>
         ...
         <tr class="order-item" v-else v-for='(item,i) in data' :key='i'>
            <td>
                <div>
                    ...
                    <p>
                        <!-- <?php echo $goods->title;?> -->{{item.title}}
                        </p>
                </div>
            </td>
            <td><span>¥ </span>
            ...
            <td><span>¥{{item.price*item.number}}</span>
            <!-- <?php echo formatMoney($goods->price * $goods->number);?> -->
            </td>
        </tr>
    ...
    <el-radio-group v-model="forms.resource">
      <el-radio label="2" value="2" style='width:180px;height:50px;'><img  style='width:130px;height:50px;margin-top:-20px;' src="../../static/img/zhifubao.png"></el-radio>
      <el-radio label="4" value="4" style='width:80px;height:50px;'><img  style='margin-top:-15px;' src="../../static/img/weixin.jpg"></el-radio>
    </el-radio-group>
  </el-form-item>
        </el-form>
    </div>
    ...
    <el-select v-model="formred.region" value-key='id' placeholder="请选择红包">
      <el-option   v-for='item in redpaper' :key='item.id' :label="item.redPaper"  :value='item'></el-option>
      ...
      <div class="total" v-if='this.$route.query.tag==5'>
        ...
        <p><span>
            <!-- <?php echo $goodsNum;?> -->
            </span>{{this.$route.query.gnumber}}件商品&nbsp;&nbsp;&nbsp;&nbsp;运费：{{data.postage}}
            <!-- <?php echo $postage;?> -->
            元</p>
        <p>总计：<i>¥</i>
    ...
    <div class="total" v-else>
        ...
        <p><span>
            <!-- <?php echo $goodsNum;?> -->
            </span>{{this.$route.query.num}}件商品&nbsp;&nbsp;&nbsp;&nbsp;运费：{{data.postage}}
            <!-- <?php echo $postage;?> -->
            元</p>
        <p>总计：<i>¥</i>
        <!-- {{data.price*this.$route.query.num+data.postage-formred.region}} -->
        <!-- {{formred.region.redPaper}} -->
         {{data.price*this.$route.query.num+parseInt(data.postage)-formred.region.redPaper}} 
        <span>
            <!-- <?php echo formatMoney($totalPrice/100);?> -->
            </span></p>
    </div>
    <div class="total_o">
        ...
        <div v-if='this.$route.query.tag==5'>
            <p>返积分：<span>
                <!-- <?php echo intval($this->setting->shoppingScore * formatMoney($totalPrice/100));?> -->
                </span>{{cartlist.score}}分</p>
                <!-- <?php $totalPrice += $postage*100;?> -->
            <p>应付金额：<i>¥</i><span >
                {{this.$route.query.gmoney+parseInt(cartlist.postage)-formred.region.redPaper}} 
                </span></p>
        </div>
        <div v-else>
            <p>返积分：<span>
  ...
  <span slot="footer" class="dialog-footer">
    <el-button @click="dialogVisible = false" class='button_t' style=' width:100px;height:40px;'>取 消</el-button>
    <el-button type="primary" @click="sure()" class='button_t' style=' width:100px;height:40px;'>已支付</el-button>
  </span>
</el-dialog>
    <div class="order_btn">
...
import { getProvince, getCity, getCounty } from "~/pages/user/account/address.js"; //
export default {
    data(){
        return{
          cartlist:'',
              ...
              if(res.data.login!=1){
                this.$router.push('/login/login')
                 ...
                 if(this.forms.resource==2){
                  ...
                  window.location='http://tapp.huainanhui.com/index.php/order_pay/index?token='+localStorage.getItem('token')+
                 '&successUrl='+'http://test.huainanhui.com'+'&errorUrl='+'http://test.huainanhui.com/pay/payfail'+'&&payment='+this.forms.resource+'&&serialNumber='+res.data.serialNumber
              ...
              if(res.data.login!=1){
                ...
                console.log(res.data)
                 ...
                 if(this.forms.resource==2){
                  ...
                  console.log(this.temp)
                  window.location='http://tapp.huainanhui.com/index.php/order_pay/index?token='+localStorage.getItem('token')+
                  '&successUrl='+this.temp+'&errorUrl='+'https://www.huainanhui.com/pay/payfail'+'&&payment='+this.forms.resource+'&&serialNumber='+res.data.serialNumber
                 }else if(this.forms.resource==4){
                              let _this=this
                              this.serialNumber=res.data.serialNumber
                  this.dialog=true
                ...
                console.log(res.data)
                 ...
                 if(this.forms.resource==2){
                  ...
                  console.log(this.temp)
                  window.location='http://tapp.huainanhui.com/index.php/order_pay/index?token='+localStorage.getItem('token')+
                  '&successUrl='+this.temp+'&errorUrl='+'https://www.huainanhui.com/'+'&&payment='+this.forms.resource+'&&serialNumber='+res.data.serialNumber
                 }else if(this.forms.resource==4){
                              let _this=this
                              this.serialNumber=res.data.serialNumber
                  this.dialog=true
                 ...
                 this.$axios.post('/api/cart/settlement',params).then((res)=>{
                  ...
                  this.redpaper=res.data.redPaper
                    ...
                    this.address=res.data.address
                     ...
                     params.append("token",localStorage.getItem('token'));
                      params.append("groupGoodsId",this.$route.query.groupGoodsId);
                         this.$axios.post('/api/group/initiate_settlement',params).then((res)=>{
                   console.log(JSON.stringify(res.data)+'yyyyyyyyyyyyyyyyy')
...
@import url('../../static/css/cart.css');
  *{margin:0;padding:0;}
    ...
    .content{min-height: 600px;width: 1200px;margin: 0 auto;padding: 0 45px;box-sizing: border-box;}
    .content>p{height: 95px;line-height: 95px;font-size: 18px;}
    .content>p span{float: left;color: #542628;}
    .content>p span:last-child{font-size: 14px;color: #74181B;float: right;}
    .info{border: 1px #e9e9e9 solid;overflow: hidden;width: 100%;}
    .info tr,.info tr td{height: 90px;}
    .info tr:nth-child(1){height: 39px;line-height: 39px;background: #f5f5f5;font-size: 14px;}
    .info tr:nth-child(1) th{font-weight: normal;}
    .info tr td{text-align: center;}
    .info tr th:nth-child(1){width: 780px;text-align: left;padding-left: 100px;box-sizing: border-box;}
    .info tr td:nth-child(1){text-align: left;padding-left: 15px;}
    .info tr td:nth-child(1) div{display: flex;flex-direction: row;}
    .info tr td:nth-child(1) div img{width: 60px;height: 60px;box-sizing: border-box;border: 1px #c9c9c9 solid;margin-right: 16px;}
    .info tr td:nth-child(1) div p{margin-top: 16px;color: #8d8d8d;font-size: 12px;}
    .info tr td input{width: 442px;height: 30px;background: #f5f5f5;outline: none;border: 0;font-size: 14px;padding: 0 14px;}
    .info tr td span{font-size: 7px;}
    .info tr td:nth-child(2),.info tr td:nth-child(4){font-size: 10px;}
    .info tr td:nth-child(4){color: #DE0812;}
    .info tr:last-child,.info tr:last-child td{height: 58px;}
    .pay_way{margin-top: 35px;overflow: hidden;}
    .pay_way p{font-size: 18px;color: #542628;margin-bottom:20px;}
    .pay_way div{width: 100%;display: flex;flex-direction: row;margin-top: 20px;padding: 0 35px;}
    .red_packet{height: 30px;margin-top: 30px;font-size: 18px;color: #542628;display: flex;flex-direction: row;margin-bottom: 44px;}
    .red_packet select{width: 348px;height: 28px;outline: none;}
    .total{height: 60px;line-height: 60px;display: flex;flex-direction: row;justify-content: flex-end;padding: 0 18px;color: #542628;}
    .total p{font-size: 18px;margin-left: 28px;}
    .total p i{font-size: 12px;}
    .total_o{height: auto;background: #f5f5f5;width: 100%;margin-bottom: 30px;}
    .total_o div{overflow: hidden;width: 100%;}
    .total_o div:nth-child(1) p{color: #5F2124;padding: 0 16px;font-size: 16px;float: left;height: 65px;line-height: 65px;}
    .total_o div:nth-child(1) p:last-child{font-size: 20px;float: right;padding: 0 16px;}
    .total_o div:nth-child(1) p:last-child i{font-size: 16px;color: #DE0812;}
    .total_o div:nth-child(1) p:last-child span{color: #DE0812;}
    .total_o div:nth-child(2) p{color: #5F2124;font-size: 16px;margin-right: 15px;}
    .total_o div:nth-child(2){display: flex;flex-direction: row;justify-content: flex-end;}
    .order_btn{height: 60px;width: 100%;margin-bottom: 300px;}
    .order_btn button{height: 60px;width: 180px;float: right;background: #74181B;color: #fff;border-radius: 5px;outline: none;border: 0;font-size: 20px;}
</style>
<style scoped>
