    ...
    <div v-if='tag==2'>
     ...
     <div v-if='tag==3'>
              <div v-if='datalista!=""'>
                   ...
                   <div class="pt_list">
                    <div class='no_order' v-if='datalist==""'> 
                        <img src="../../static/img/noOrder.jpg" alt />
            <p>当前还没有人发起拼团哦，<br>从我开始吧！</p></div>
                    <ul class="ull">

						<li v-for='(item,i) in datalist' :key='i'> 用户：<span><img :src="item.avatar"/>{{item.userName}}</span><i>&nbsp;&nbsp;&nbsp;</i>发起团购，距离结束还有
                        <span >{{item.time}}</span> ,还差{{item.goodsNum-item.joinNum}}人拼成。 <span @click='join(item.id)'>参与拼团</span></li>
