     ...
     <div v-if='tag==3'>
            ...
            <p>当前还没有人发起拼团哦，<br>从我开始吧！</p></div>
                    <!-- <ul class="ull"> -->
						...
						<li v-for='(item,i) in datalistjj' :key='i'> 用户：<span><img :src="item.avatar"/>{{item.userName}}</span><i>&nbsp;&nbsp;&nbsp;</i>参与竞拍，
                        <p style='float:right;margin-right:40px;'>出价:{{item.price}}</p>
                        </li>
...
con1.style.marginTop='-100px';
this.animate=!this.animate;
let that = this; // 在异步函数中会出现this的偏移问题，此处一定要先保存好this的指向
       this.animate=true;    // 因为在消息向上滚动的时候需要添加css3过渡动画，所以这里需要设置true
       setTimeout(()=>{      //  这里直接使用了es6的箭头函数，省去了处理this指向偏移问题，代码也比之前简化了很多
               this.datalistjj.push(this.datalistjj[0],this.datalistjj[1],this.datalistjj[3]);  // 将数组的第一个元素添加到数组的
               this.datalistjj.shift(0,3);               //删除数组的第一个元素
