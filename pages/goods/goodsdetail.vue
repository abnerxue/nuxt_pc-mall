<template>
        ...
        <div class="gt_right">
          ...
          <div v-if='tag==1'>
              <div v-if='datalist!=""'>
                   ...
                   <div class="pt_list">
                    <div class='no_order' v-if='datalistjj==""'> 
                        <img src="../../static/img/noOrder.jpg" alt />
            <p>当前还没有人发起拼团哦，<br>从我开始吧！</p></div>
                    <!-- <ul class="ull">

						<li v-for='(item,i) in datalistjj' :key='i'> 用户：<span><img :src="item.avatar"/>{{item.userName}}</span><i>&nbsp;&nbsp;&nbsp;</i>发起团购，距离结束还有
					...
					<li v-for='(item,i) in datalistjj' :key='i'> <span><img :src="item.avatar"/>{{item.userName}}</span><i>&nbsp;&nbsp;&nbsp;</i>发起团购，距结束还有
                        <span >{{item.time}}</span> ,差{{item.goodsNum-item.joinNum}}人拼成。 <span @click='join(item.id)'>参与拼团</span></li>
              ...
              <div v-if='datalist!=""'>
                   ...
                   <div class="pt_list">
                    <div class='no_order' v-if='datalista==null'> 
                        <img src="../../static/img/noOrder.jpg" alt />
            <p>当前还没有人出价哦，<br>开始出价吧！</p></div>
                    <!-- <ul class="ull"> -->
                         <ul id="con1" ref="con1" :class="{anim:animate==true}"  @mouseenter="mEnter" @mouseleave="mLeave">
						<li v-for='(item,i) in datalistjj' :key='i'> 用户：<span><img :src="item.avatar"/>{{item.userName}}</span><i>&nbsp;&nbsp;&nbsp;</i>{{item.postTime}}参与竞拍，
                       <i>&nbsp;&nbsp;&nbsp;</i> <p style='float:right;margin-right:40px;'>出价:{{item.price}}</p>
                        </li>
					...
					<div class="team_w">
                     ...
                     <div class="g_buy">
                         <p style='margin-top:-20px;color:#74181b;margin-bottom:10px;'>当前最高出价{{this.heighest}}元</p>
                    <button class="buy" click="">购买</button>
                    <button class="joincart" @click='compet()'>参与竞价</button>
                  
                </div>
                <p class='illustration' @click='dialogVisible = true'>注：参与竞价需先缴纳20元保证金，活动结束后返还.更多...></p>
                 <p> {{datalist.title}}</p>
                <div class="g_detail">
                    <p>
                        <span>起拍价</span>
                <span><i>¥</i>{{datalistj.startingPrice}}</span>
                        <del>原价<i>{{datalist.price}}</i></del>
                    </p>
                    <p>
                        <span>运费</span>
                        <span><i>满免</i>满 元免运费</span>
  ...
  width="30%"
  >
 保证金说明    竞买人需支付30元保证金方可参与拍卖，在竞拍倒计时结束前均可缴纳保证金参与竞拍，竞拍结束后，除买受人外，其他竞买人缴纳保证金将即刻解冻，资金将在三个工作日内由原渠道退回。买受人若未在48小时内支付所有款项，本平台将扣除之前所缴纳保证金，不予退还。若同时竞拍多个商品，则须缴纳多份保证金（30元/份），以保证顺利竞拍所有您想要竞拍的商品。     竞拍规则         拍卖开始后，已缴纳保证金的竞买人可以进行出价操作，竞买人须按一定要求竞价（如：固定初始竞价、固定加价幅度），不得在出价已领先情况下连续出价，扰乱竞拍秩序。
...
<el-dialog
  ...
  width="30%"
 >
  <el-input v-model="input" width='80%' placeholder="请输入您的出价(元)"></el-input>
  <span slot="footer" class="dialog-footer">
    <el-button @click="dialogVisiblej = false">取 消</el-button>
    <el-button type="primary" @click="offer">确 定</el-button>
  </span>
</el-dialog>
      <!-- <div v-if='tag==3'>
            <div class="g_detail">
...
this.timer1= setInterval(this.scroll,2000)
          ...
          scroll(){
              ...
              if(this.datalistjj.length>3){
                 let con1 = this.$refs.con1;
con1.style.marginTop='-85px';
this.animate=!this.animate;

let that = this; // 在异步函数中会出现this的偏移问题，此处一定要先保存好this的指向
   
       setTimeout(()=>{      //  这里直接使用了es6的箭头函数，省去了处理this指向偏移问题，代码也比之前简化了很多
               that.datalistjj.push(this.datalistjj[0]);  // 将数组的第一个元素添加到数组的
               that.datalistjj.shift();               //删除数组的第一个元素
...
that.animate=!that.animate;
      ...
      let params = new URLSearchParams();
       params.append("token",localStorage.getItem('token'));
        params.append("aid",this.datalistj.id);
         params.append("price",this.input);
      this.$axios.post("/api/auction/bidding_price",params).then(res => {
                          this.$message({
      ...
      this.input=''
        ...
        compet(){
            ...
            if(localStorage.getItem('login')!=1){
                ...
                if(this.datalistj.deposit==0){
                                 this.$message({
          message: '您还没缴纳保证金，需先缴纳保证金'+this.datalistj.bond+'元',
          type: 'success',
          offset: "400",
         duration: "1000"
        });
      let params={'token':localStorage.getItem('token'),'aid':this.datalistj.id,'payment':2}
          // 创建form元素
    var temp_form = document.createElement("form");
    // 设置form属性
    temp_form .action = 'http://tapp.huainanhui.com/index.php/api/auction/bond'      
    ...
    if(this.dataseckill.endTime!=''){
      ...
      this.interval = setTimeout(function() {
        that.countdowns();
           ...
           countdown() {
      if(this.datalistjj.endTime!=''){
       
             const end = Date.parse(new Date(this.datalistjj.endTime));
      const now = Date.parse(new Date());
      const msec = end - now;
      //  console.log(this.datalistjj.endTime)
      ...
      const that = this;
      this._intervalj = setTimeout(function() {
        that.countdownj();
      }, 1000);
      }
    },
      ...
      this.$axios.get("/api/pc/index").then(res => {
        ...
        // this.datalistjj = res.data.group;
        ...
        show(){
            ...
            if(this.$route.query.tag==1){
                 this.$axios.get('/api/group/group_goods_detail?groupGoodsId='+this.$route.query.id+'&token='+localStorage.getItem('token')).then((res)=>{
                    ...
                    //  this.datalistjj=Object.values(res.data.initiateList[0]);
                    res.data.initiateList.forEach(i => {
                          i.time=this.private(i.timeRemaining)
                    });
                        this.datalistjj=Object.values(res.data.initiateList);
                ...
                //    console.log(this.datalistj.endTime+'this.datalists')
                         ...
                         let params = new URLSearchParams();
                     params.append("token",localStorage.getItem('token'));
                      params.append("aid",this.$route.query.id);
                      this.$axios.post('/api/auction/bidding_list',params).then((res)=>{
                     this.datalistjj=res.data.list
                     this.heighest=res.data.list[0].price
                   console.log(res.goods)
                 })
 ...
 mounted(){
   this.timer1= setInterval(this.scroll,2000)
        console.log(JSON.stringify(this.$route.query)+'fff')
     this.countdown()
       this.countdowns()
        this.countdownj()
     ...
     this.show(),
     this. getcomment()
     console.log(this.datalistjj+'7788')
